...
- **1 untested route:**
  - `GET /v1/communities/:communityId/emoji/:emojiId/content` — serves the binary content of a custom server emoji. The upload test (line 1240) asserts the returned `url` _contains_ this path, but no test actually fetches the endpoint. The list and delete emoji tests also do not exercise it.
